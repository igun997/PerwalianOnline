$(document).ready(function() {
  console.log("sekretariat Running");
  table_main = $("#main").DataTable({
    ajax:base_url+"jurusan/readsekretariat"
  })
  $("#addsekretariat").on('click', function(event) {
    ...
    dialog.init(function() {
      setTimeout(function() {
        dialog.find(".modal-title").html("Tambah sekretariat");
        form = [[{
          label:"Nama Lengkap",
          type:"text",
          name:"nama_lengkap",
        ...
        dialog.find("#save").on('submit', function(event) {
         ...
         ins = post(base_url+"jurusan/addsekretariat",dform);
         if (ins.status == 1) {
  ...
  $("#main").on('click',".updatesekretariat", function(event) {
    event.preventDefault();
    id = $(this).data("id");
    getdata = post(base_url+"jurusan/detailsekretariat",{id_user:id});
    if (getdata.status == 1) {
      var dialog = bootbox.dialog({
        title: 'Prepare Menu ',
        message: '<p><center><i class="fa fa-spin fa-spinner"></i> Loading...</center></p>'
      ...
      dialog.init(function() {
        setTimeout(function() {
          dialog.find(".modal-title").html("Ubah sekretariat");
          form = [[{
            label:"Nama Lengkap",
            type:"text",
            name:"nama_lengkap",
          ...
          dialog.find("#save").on('submit', function(event) {
            ...
            toastr.info("Data Sedang Dikirim");
            ins = post(base_url+"jurusan/upsekretariat",dform);
            if (ins.status == 1) {
              table_main.ajax.reload();
              toastr.success(ins.msg);
              bootbox.hideAll();
  ...
  $("#main").on('click', '.deletesekretariat', function(event) {
    event.preventDefault();
    if (confirm("Apakah Anda Yakin ?")) {
      id = $(this).data("id");
      console.log(id);
      del = post(base_url+"jurusan/delsekretariat",{id_user:id});
      if (del.status == 1) {
        table_main.ajax.reload();
